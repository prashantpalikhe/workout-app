...
// ────────────────────────────────────────────
// Exercise Media (imageUrl + instructions)
//   Generated by prisma/scripts/sync-exercise-images.ts.
//   Safe to commit or regenerate.
// ────────────────────────────────────────────
...
function loadExerciseMedia(): Record<string, ExerciseMedia> {
  const mediaPath = join(__dirname, 'data', 'exercise-media.json');
  if (!existsSync(mediaPath)) {
    console.log('  ℹ No exercise-media.json found, skipping media fields');
    return {};
  }
  return JSON.parse(readFileSync(mediaPath, 'utf-8'));
}
...
async function seedExercises() {
  ...
  const withMedia = Object.keys(media).length;
  console.log(`  ✓ ${count} global exercises (${withMedia} with media)`);
}
